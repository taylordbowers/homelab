...
- **Web UI:** port 8006 on each node, fronted via NPM with TLS
...
A Claude Code MCP server runs in CT 102 (claude LXC) on pve2. This enables AI-assisted cluster management — creating/deleting VMs, checking status, running commands inside VMs — directly from a Claude conversation. CT 102 also hosts an internal documentation wiki used as a long-term archive of all changes and decisions.
